...
void CCockpit2View::DrawArrow(CDC * pDC, CRect rc, CSize szArrow)
{
	double offsetX = (double)szArrow.cx / 100;
	double offsetY = (double)szArrow.cy / 100;

	CPoint points[7] =
	{
		CPoint(rc.left + rc.Width() / 2, rc.top), // gornja srednja tacka
		CPoint(rc.left, rc.top + rc.Height() * offsetY), // leva tacka na rect-u
		CPoint(rc.left + rc.Width() * offsetX, rc.top + rc.Height() * offsetY), // leva srednja tacka
		CPoint(rc.left + rc.Width() * offsetX, rc.bottom), // leva donja tacka
		CPoint(rc.right - rc.Width() * offsetX, rc.bottom), // desna donja tacka
		CPoint(rc.right - rc.Width() * offsetX, rc.top + rc.Height() * offsetY), // desna gornja tacka
		CPoint(rc.right, rc.top + rc.Height() * offsetY), // desna na rect-u
		//CPoint(rc.left + rc.Width() / 2, rc.top),
	}; 
...
void CCockpit2View::nacrtajAvion(CDC * pDC, CRect wall, CString putanja)
{
	int width = wall.Width();
	int height = wall.Height();
	...
	pDC->StretchBlt(width / 3, height / 3, bm.bmWidth / 8, bm.bmHeight / 8, MemDC, 0, 0, bm.bmWidth, bm.bmHeight, SRCAND);


	MemDC->SelectObject(&bmpImage);

	//pDC->BitBlt(0,0,bm.bmWidth,bm.bmHeight,MemDC,0,0,SRCPAINT);
	pDC->StretchBlt(width / 3, height / 3, bm.bmWidth / 8, bm.bmHeight / 8, MemDC, 0, 0, bm.bmWidth, bm.bmHeight, SRCPAINT);
...
void CCockpit2View::IzotropniMapping(CDC * pDC)
{
	...
	//test/////////////////////////
	p1.SetPoint(zid.Width() * 0.698, zid.Height() * 0.83);
	p2.SetPoint(zid.Width() * 0.77, zid.Height() * 0.978);
	...
	nacrtajSat(pDC, zid, leviSat, 30, vrednosti1, 8, 40, 340, 2, 60);

	//Levi gornji sat za kompas
	int leviGornjiSatX = zid.Width() / 2 - 2.8*width;
	int leviGornjiSatY = 15 * p*zid.Height() - 2;
	////////////test///////
	p1.SetPoint(zid.Width() * 0.305, zid.Height() * 0.78);
	p2.SetPoint(zid.Width() * 0.372, zid.Height() * 0.93);
	CRect leviGornjiSat(p1, p2);
	///////////////////////
	...
	nacrtajSat(pDC, zid, leviGornjiSat, 16, vrednosti2, 8, 0, 320, 3, 30);

	//Desni gornji sat ziroskop 
	int desniGornjiSatX = zid.Width() / 2 + 1.8*width;
	int desniGornjiSatY = 15 * p*zid.Height() - 2;
	//test//////////////
	p1.SetPoint(zid.Width() * 0.623, zid.Height() * 0.78);
	p2.SetPoint(zid.Width() * 0.692, zid.Height() * 0.93);
	CRect desniGornjiSat(p1, p2);
	//////////////////////
	...
	size.cx = 20;
	size.cy = 40;
	p1.SetPoint(zid.Width() * 0.531, zid.Height() * 0.26);
	p2.SetPoint(zid.Width() * 0.6, zid.Height() * 0.46);
	watch.SetRect(p1, p2);
	DrawArrow(pDC, watch, size);
}
...
void CCockpit2View::DrawPoint(CDC * pDC, CRect wall)
{
	...
	{
		CPoint(wall.Width() * 0.531, wall.Height() * 0.26),
		CPoint(wall.Width() * 0.6, wall.Height() * 0.46),
